  ...
  return {
    ...
    downloader: {
      ...
      async create(feed, opt = {}) {
        ...
        var [err, data] = await WXU.request({
          ...
          body: {
            ...
            createtime: String(feed.createtime || Math.floor(Date.now() / 1000)),
            key: String(feed.key || ""),
          },
        });
        if (err) {
      ...
      async create_batch(feeds, opt = {}) {
        var username = "";
        if (feeds.length > 0 && feeds[0].contact) {
          username = feeds[0].contact.id || feeds[0].contact.username || "";
        }
        if (username) {
          await WXU.request({
            method: "POST",
        ...
        for (let i = 0; i < feeds.length; i += 1) {
          ...
          try {
            ...
            console.error("[downloader.create_batch] error", e);
          }
        }
        return [null, { ids: created_ids }];
      },
    },
    /**
     * 视频解密
