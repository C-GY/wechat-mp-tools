/** Guangce hub configuration, scheduler, and durable progress dashboard. */
const ChannelsGuangcePage = {
    pollTimer: null,
    scheduleTimes: [],
    ...
    render() {
        return `
            <div class="page-header animate-fade-in" style="display:flex; justify-content:space-between; align-items:flex-start; gap:16px; flex-wrap:wrap;">
                <div>
                    <h2 class="page-title">广策中枢系统</h2>
                    <p class="page-description">按创作者逐个刷新作品、增量同步 OSS；按同步批次保留视频快照，同批次重试更新原快照。</p>
                </div>
                <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                    <button class="btn btn-secondary" id="btn-guangce-export-config" onclick="ChannelsGuangcePage.exportConfig()" title="复制已保存的完整配置，包含数据库密码和飞书密钥">📋 导出配置</button>
                    <button class="btn btn-secondary" id="btn-guangce-import-config" onclick="ChannelsGuangcePage.importConfig()">📥 导入配置</button>
                    <button class="btn btn-primary" id="btn-guangce-run" onclick="ChannelsGuangcePage.startRun()">▶ 立即执行</button>
                    <button class="btn btn-secondary" id="btn-guangce-pause" data-action="pause" onclick="ChannelsGuangcePage.togglePause()" style="display:none;">⏸ 暂停</button>
                </div>
            </div>

            <div id="guangce-summary" class="card animate-fade-in" style="margin-top:var(--spacing-lg); padding:18px;">
                <div style="display:flex; justify-content:space-between; gap:12px; flex-wrap:wrap; align-items:center;">
                    <div>
                        <div style="font-size:.78rem; color:var(--text-muted);">当前状态</div>
                        <div id="guangce-run-title" style="font-size:1.05rem; font-weight:700; margin-top:4px;">正在读取...</div>
                    </div>
                    <div id="guangce-next-run" style="font-size:.82rem; color:var(--text-muted);">下次执行：—</div>
                </div>
                <div style="height:8px; border-radius:999px; overflow:hidden; background:rgba(0,0,0,.08); margin-top:14px;">
                    <div id="guangce-main-progress" style="height:100%; width:0; background:var(--primary); transition:width .25s;"></div>
                </div>
                <div id="guangce-run-message" style="font-size:.84rem; color:var(--text-secondary); margin-top:10px;">—</div>
                <div id="guangce-current-creator" style="display:none; align-items:center; gap:10px; margin-top:12px; padding:10px 12px; border-radius:9px; background:rgba(37,99,235,.08); color:var(--text-secondary);">
                    <span id="guangce-current-creator-position" style="font-size:.78rem; white-space:nowrap;">当前创作者</span>
                    <strong id="guangce-current-creator-name" style="color:var(--text-primary); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">—</strong>
                </div>
                <div class="guangce-stats-row">
                    ${this.statCard('当前创作者', 'guangce-stat-creators', 'guangce-stat-creator-name')}
                    ${this.statCard('刷新作品', 'guangce-stat-refreshed')}
                    ${this.statCard('数据库已有', 'guangce-stat-existing')}
                    ${this.statCard('新增作品', 'guangce-stat-new')}
                    ${this.statCard('OSS 完成', 'guangce-stat-uploaded')}
                    ${this.statCard('数据库处理', 'guangce-stat-written')}
                    ${this.statCard('失败项', 'guangce-stat-failed')}
                </div>
            ...
            <div class="card animate-fade-in" style="margin-top:var(--spacing-lg);">
                <div class="card-header" style="border-bottom:1px solid var(--border-color); padding-bottom:var(--spacing-md); margin-bottom:var(--spacing-md);">
                    ...
                    <div style="font-size:.8rem; color:var(--text-muted); margin-top:5px;">写入所配置数据库的 competitor_video_snapshots 表；配置保存在当前电脑用户目录，重装后不会丢失。</div>
                </div>
                <div style="display:grid; grid-template-columns:minmax(220px,2fr) minmax(120px,1fr); gap:14px;">
                    ${this.input('数据库地址', 'guangce-db-host', 'text', '例如 127.0.0.1')}
                    ${this.input('端口', 'guangce-db-port', 'number', '3306')}
                </div>
                <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:14px; margin-top:14px;">
                    ${this.input('账号', 'guangce-db-username', 'text', 'MySQL 用户名')}
                    ${this.input('密码', 'guangce-db-password', 'password', '请输入密码')}
                    ${this.input('数据库', 'guangce-db-database', 'text', '请输入数据库名称')}
                </div>
                <div style="display:flex; gap:8px; margin-top:16px; flex-wrap:wrap;">
                    <button class="btn btn-primary" id="btn-guangce-save" onclick="ChannelsGuangcePage.saveConfig()">保存全部配置</button>
                    <button class="btn btn-secondary" id="btn-guangce-test-db" onclick="ChannelsGuangcePage.testDatabase()">测试 MySQL</button>
                </div>
            </div>
            ...
            <div class="card animate-fade-in" style="margin-top:var(--spacing-lg);">
                ...
                <label style="display:flex; align-items:center; gap:9px; cursor:pointer; font-weight:600;">
                    <input id="guangce-schedule-enabled" type="checkbox"> 启用定时同步
                </label>
                <div style="display:flex; gap:8px; align-items:flex-end; margin-top:14px; flex-wrap:wrap;">
                    <div class="form-group" style="margin:0;">
                        <label class="form-label" for="guangce-new-time">新增触发时间</label>
                        <input id="guangce-new-time" class="form-input" type="time" value="09:00" style="width:170px;">
                    </div>
                    <button class="btn btn-secondary" onclick="ChannelsGuangcePage.addTime()">＋ 添加时间</button>
                    <div class="form-group" style="margin:0; min-width:220px;">
                        <label class="form-label" for="guangce-creator-interval">创作者之间的间隔（秒）</label>
                        <input id="guangce-creator-interval" class="form-input" type="number" min="0" max="86400" value="10">
                    </div>
                </div>
                <div id="guangce-time-list" style="display:flex; gap:8px; flex-wrap:wrap; margin-top:14px;"></div>
                <div style="display:flex; gap:10px; align-items:center; margin-top:16px; flex-wrap:wrap;">
                    <button class="btn btn-primary" id="btn-guangce-save-schedule" onclick="ChannelsGuangcePage.saveConfig()">保存定时配置</button>
            ...
            <div class="card animate-fade-in" style="margin-top:var(--spacing-lg);">
                ...
                ${this.input('Webhook 地址', 'guangce-feishu-webhook', 'url', 'https://open.feishu.cn/open-apis/bot/v2/hook/...')}
                <div style="margin-top:14px;">${this.input('签名密钥（可选）', 'guangce-feishu-secret', 'password', '机器人未开启签名校验可留空')}</div>
                <div style="display:flex; gap:8px; margin-top:16px; flex-wrap:wrap;">
                    <button class="btn btn-secondary" id="btn-guangce-test-feishu" onclick="ChannelsGuangcePage.testFeishu()">发送测试消息</button>
                    <button class="btn btn-secondary" onclick="Router.navigate('channels_oss_config')">打开 OSS 配置</button>
                    <span id="guangce-oss-state" style="font-size:.82rem; color:var(--text-muted); align-self:center;">OSS 状态：读取中</span>
                </div>
            </div>
            ...
            <div class="card animate-fade-in" style="margin-top:var(--spacing-lg);">
                <div class="card-header" style="display:flex; justify-content:space-between; align-items:center; gap:10px; border-bottom:1px solid var(--border-color); padding-bottom:var(--spacing-md); margin-bottom:var(--spacing-md);">
                    ...
                    <button class="btn btn-secondary btn-sm" onclick="ChannelsGuangcePage.loadStatus()">刷新</button>
                </div>
                <div style="overflow:auto;">
                    <table class="data-table" style="min-width:1100px; width:100%;">
                        ...
                        <tbody id="guangce-creator-rows"><tr><td colspan="9" style="text-align:center; padding:32px; color:var(--text-muted);">暂无执行记录</td></tr></tbody>
                    </table>
                </div>
            </div>
            ...
            <div class="card animate-fade-in" style="margin-top:var(--spacing-lg); margin-bottom:var(--spacing-lg);">
                ...
                <div id="guangce-history" style="display:grid; gap:9px;"><div style="color:var(--text-muted);">暂无运行记录</div></div>
            </div>
        `;
    },
    ...
    async loadConfig() {
        try {
            const config = await API.guangce.getConfig();
            const db = config.database || {};
            const schedule = config.schedule || {};
            const feishu = config.feishu || {};
            this.setValue('guangce-db-host', db.host || '');
            this.setValue('guangce-db-port', db.port || 3306);
            this.setValue('guangce-db-username', db.username || '');
            this.setValue('guangce-db-database', db.database || '');
            this.setSecretPlaceholder('guangce-db-password', db.has_password, '数据库密码');
            this.setValue('guangce-creator-interval', schedule.creator_interval_seconds ?? 10);
            const enabled = document.getElementById('guangce-schedule-enabled');
            if (enabled) enabled.checked = !!schedule.enabled;
            this.scheduleTimes = Array.isArray(schedule.times) ? [...schedule.times] : [];
            this.renderTimes();
            this.setSecretPlaceholder('guangce-feishu-webhook', feishu.has_webhook, '飞书机器人 Webhook');
            this.setSecretPlaceholder('guangce-feishu-secret', feishu.has_secret, '飞书签名密钥');
            const oss = document.getElementById('guangce-oss-state');
            if (oss) {
            ...
            Toast.error(`读取广策中枢配置失败：${error.message || error}`);
        }
    },

    exportConfig() {
        return HubConfigTransfer.exportConfig({
            api: API.guangce, label: '广策中枢', buttonId: 'btn-guangce-export-config',
        });
    },

    importConfig() {
        HubConfigTransfer.importConfig({
            api: API.guangce, label: '广策中枢',
    ...
    collectConfig() {
        return {
            database: {
                host: this.value('guangce-db-host'),
                port: Number(this.value('guangce-db-port') || 3306),
                username: this.value('guangce-db-username'),
                password: this.value('guangce-db-password'),
                database: this.value('guangce-db-database'),
            },
            schedule: {
                enabled: !!document.getElementById('guangce-schedule-enabled')?.checked,
                times: [...this.scheduleTimes],
                creator_interval_seconds: Number(this.value('guangce-creator-interval') || 0),
            },
            feishu: {
                webhook_url: this.value('guangce-feishu-webhook'),
                secret: this.value('guangce-feishu-secret'),
            },
        };
    },
    ...
    async saveConfig(showToast = true) {
        const buttons = [
            [document.getElementById('btn-guangce-save'), '保存全部配置'],
            [document.getElementById('btn-guangce-save-schedule'), '保存定时配置'],
        ...
        try {
            ...
            const result = await API.guangce.saveConfig(this.collectConfig());
    ...
    async testDatabase() {
        ...
        const button = document.getElementById('btn-guangce-test-db');
        try {
            if (button) { button.disabled = true; button.textContent = '连接中...'; }
            const result = await API.guangce.testDatabase();
    ...
    async testFeishu() {
        if (!await this.saveConfig(false)) return;
        const button = document.getElementById('btn-guangce-test-feishu');
        try {
            if (button) { button.disabled = true; button.textContent = '发送中...'; }
            const result = await API.guangce.testFeishu();
    ...
    async startRun() {
        if (!await this.saveConfig(false)) return;
        const button = document.getElementById('btn-guangce-run');
        let started = false;
        try {
            if (button) { button.disabled = true; button.textContent = '正在创建任务...'; }
            const result = await API.guangce.startRun();
            started = true;
    ...
    async togglePause() {
        const button = document.getElementById('btn-guangce-pause');
        ...
        try {
            ...
            const result = action === 'resume'
                ? await API.guangce.resumeRun()
                : await API.guangce.pauseRun();
    ...
    addTime() {
        const input = document.getElementById('guangce-new-time');
    ...
    renderTimes() {
        const list = document.getElementById('guangce-time-list');
        if (!list) return;
        if (!this.scheduleTimes.length) {
            list.innerHTML = '<span style="font-size:.82rem; color:var(--text-muted);">尚未添加触发时间</span>';
            return;
        }
        list.innerHTML = this.scheduleTimes.map(value => `<span class="badge" style="display:inline-flex; gap:8px; align-items:center; padding:6px 10px; background:rgba(7,193,96,.08); color:var(--text-primary);">${this.esc(value)} <button type="button" aria-label="删除 ${this.attr(value)}" onclick="ChannelsGuangcePage.removeTime('${this.attr(value)}')" style="border:0; background:none; cursor:pointer; color:var(--error); padding:0;">×</button></span>`).join('');
    },

    async loadStatus() {
        try {
            const status = await API.guangce.getStatus();
    ...
    renderStatus(status) {
        ...
        const title = document.getElementById('guangce-run-title');
        const message = document.getElementById('guangce-run-message');
        const progress = document.getElementById('guangce-main-progress');
        const next = document.getElementById('guangce-next-run');
        const button = document.getElementById('btn-guangce-run');
        const pauseButton = document.getElementById('btn-guangce-pause');
        const currentCreator = document.getElementById('guangce-current-creator');
        const currentCreatorPosition = document.getElementById('guangce-current-creator-position');
        const currentCreatorName = document.getElementById('guangce-current-creator-name');
        ...
        this.text('guangce-stat-creators', `${hasCurrentCreator ? currentIndex : done}/${total}`);
        const creatorStatName = document.getElementById('guangce-stat-creator-name');
        if (creatorStatName) {
            creatorStatName.textContent = currentName || '—';
            creatorStatName.title = currentName;
        }
        this.text('guangce-stat-refreshed', run.refreshed_videos || 0);
        this.text('guangce-stat-existing', run.existing_videos || 0);
        this.text('guangce-stat-new', run.new_videos || 0);
        this.text('guangce-stat-uploaded', run.uploaded_videos || 0);
        this.text('guangce-stat-written', run.database_written || 0);
        this.text('guangce-stat-failed', run.failed_items || 0);
    ...
    renderCreatorRows(items, run = {}, running = false) {
        const body = document.getElementById('guangce-creator-rows');
        if (!body) return;
        const total = Number(run.total_creators || 0);
        const currentIndex = Number(run.current_creator_index || 0);
    ...
    renderHistory(items) {
        const container = document.getElementById('guangce-history');
        if (!container) return;
        if (!items.length) {
