...
def make_proxy_manager(*, running):
    manager = Mock(running=running)
    manager.recover_if_unhealthy.side_effect = lambda: False if manager.running else manager.start()
    return manager
...
class ChannelsEnvironmentTests(unittest.TestCase):
    ...
    def test_existing_native_browser_without_heartbeat_is_not_reopened(self):
        ...
        with (
            patch.object(mitm_proxy.ProxyManager, "get_instance", return_value=make_proxy_manager(running=True)),
    ...
    def test_diagnostic_endpoint_does_not_change_proxy_or_windows(self):
        ...
        self.browser_windows.return_value = [{"hwnd": 42, "minimized": True}]
        manager = make_proxy_manager(running=False)
        with (
    ...
    def test_pipeline_waits_for_existing_page_before_trying_to_open_wechat(self):
        manager = make_proxy_manager(running=True)
    ...
    def test_diagnostics_omit_window_titles_and_page_identifiers(self):
        ...
        with patch.object(mitm_proxy.ProxyManager, "get_instance", return_value=make_proxy_manager(running=True)):
            result = wechat_automation.ensure_wechat_channels_available(detection_timeout=0, open_timeout=0)
    ...
    def test_failed_proxy_recovery_waits_without_touching_wechat(self):
        manager = make_proxy_manager(running=True)
        manager.recover_if_unhealthy.side_effect = RuntimeError("采集代理尚未恢复，将自动重试")
        with (
            patch.object(mitm_proxy.ProxyManager, "get_instance", return_value=manager),
            patch.object(wechat_automation, "open_wechat_video_channels") as open_channels,
            patch.object(wechat_automation, "_reload_wechat_browser_window") as reload,
        ):
            with self.assertRaisesRegex(RuntimeError, "自动重试"):
                wechat_automation.ensure_wechat_channels_available(
                    detection_timeout=0, open_timeout=0, recover_browser=True)
        open_channels.assert_not_called()
        reload.assert_not_called()
        events = [json.loads(line)["event"] for line in
                  (self.log_dir / "channels_environment.jsonl").read_text(encoding="utf-8").splitlines()]
        self.assertIn("proxy_recovery_failed", events)

    def test_rebuilt_proxy_alone_does_not_allow_collection_without_page_heartbeat(self):
        self.browser_windows.return_value = [{"hwnd": 42, "minimized": False}]
        manager = make_proxy_manager(running=True)
        manager.recover_if_unhealthy.side_effect = None
        manager.recover_if_unhealthy.return_value = True
        with (
            patch.object(mitm_proxy.ProxyManager, "get_instance", return_value=manager),
            patch.object(wechat_automation, "open_wechat_video_channels") as open_channels,
            patch.object(wechat_automation, "_reload_wechat_browser_window", return_value=True),
        ):
            result = wechat_automation.ensure_wechat_channels_available(
                detection_timeout=0, open_timeout=0, recover_browser=True)
        self.assertFalse(result["monitoring_active"])
